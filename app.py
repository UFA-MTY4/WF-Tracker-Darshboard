...
valid_time_cols = [c for c in time_columns if c in df_filtered.columns]

if not valid_time_cols:
    st.info("⚠️ No time-related columns available in the filtered dataset.")
else:
    # Asegurar conversión numérica
    df_time = df_filtered[valid_time_cols].apply(pd.to_numeric, errors="coerce")
...
with cb:
    # Validamos que avg_df exista y tenga datos para la gráfica de tiempos promedio
    if 'avg_df' in locals() and not avg_df.empty:
        fig_time = px.bar(
            ...
            y="Metric",
            orientation="h",
            title="Average Time Metrics (Filtered)",
            text="Average",
            color="Average",
